"""Binary spray and wait over an intermittently connected mobile network (DTN) - Unicast Scenario."""
...
for nid in 'ABCDEFGH':
    net.add_node(binary_spray_and_wait(nid, L=4), 0, 0)
...
def status():
    ...
    if 'file.pdf' in net.nodes[DESTINATION].buffer:
        print(f"\n  [Success!] (t={loop.time:.1f}): Destination node '{DESTINATION}' successfully received the file.\n")
        enc.stop()
        done = True
    else:
        have = sorted(n for n in net.nodes if net.nodes[n].buffer)
        print(f"  Status (t={loop.time:.1f}): Node {DESTINATION} waiting. Nodes currently carrying copies: {have}")
